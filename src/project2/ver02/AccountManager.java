...
//	// 입금
	public void depositMoney()
	{
		...
		int inmoney = s3.nextInt();
		for (int i = 0; i < accNum; i++)
		{
			if (search.equals(SaveAccount[i].account) && SaveAccount[i].getLevel().equals("A"))
			{
			...
			} else if (search.equals(SaveAccount[i].account) && SaveAccount[i].getLevel().equals("B"))
			{
				SaveAccount[i].money = (int) (SaveAccount[i].money
						+ (SaveAccount[i].money * ((double) SaveAccount[i].getMaxinterest() / 100))
						+ ((double) SaveAccount[i].money * B) + inmoney);
				System.out.println("입금이 완료되었습니다.");
			} else if (search.equals(SaveAccount[i].account) && SaveAccount[i].getLevel().equals("C"))
			{
				SaveAccount[i].money = (int) (SaveAccount[i].money
						+ (SaveAccount[i].money * ((double) SaveAccount[i].getMaxinterest() / 100))
						+ ((double) SaveAccount[i].money * C) + inmoney);
				System.out.println("입금이 완료되었습니다.");
			} else if (search.equals(SaveAccount[i].account))
			{
				SaveAccount[i].money = (int) (SaveAccount[i].money
